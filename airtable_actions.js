 ...
 * @returns {object} - {Name, Links, Notes}
 */
const lc2List = (lc) => {
  let Links = [],
    lines,
  ...
  lines.forEach((l) => {
    ...
    if (((m = l.match(/^- \[(.+)]\((.+?)\)$/)), m)) {
      ...
      Links[Links.length - 1].Notes = m[1];
    }
  });
  return { Name, Links, Notes };
};

/**
 *
 * @param {object} list
 * @example list = {Name, Links, Notes}
 */
const addList = async (list) => {
  ...
  if (list.Name) {
    ...
    if (list.Notes) {
      ...
      Notes += (Notes ? "\n" : "") + list.Notes;
      await updateRecord(tables.Links.id, [{ id: listId, fields: { Notes } }]);
    }
  }
...
const list2Lc = async (listId) => {
  ...
  if (!list) return;
  let lc = list.Name + ".md";
  let content = list.Notes ? "> " + list.Notes + "\n\n" : "";
