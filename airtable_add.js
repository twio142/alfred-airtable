...
const ask = async (input = "") => {
  ...
  let { url, title, table, type, tags, notes, list, id, done } = process.env;
  done = done ? done * 1 : 0;
  if (!url) {
  ...
  if (!url) {
    ...
    if (testURL(input)) {
      items.push({
        ...
        variables: { url: input, title, table, type, tags, notes, list, done },
      });
    } else {
      items.push({
        title: url,
    ...
    if (input || title) {
      items.push({
        ...
        variables: { url, title: input, table, type, tags, notes, list, done },
      });
    }
    ...
    if (done) {
      items.push({
        ...
        variables: { url, title, table, type, tags, notes, list, done: 0 },
      });
    } else if (input == ".d") {
      items.push({
        ...
        arg: " ",
        icon: { path: "media/checked.png" },
        variables: { url, title, table, type, tags, notes, list, done: 1 },
      });
    }
    if (!notes && input) {
      items.push({
        ...
        variables: { url, title, table, type, tags, notes: input, list },
      });
    }
    items.unshift({
      title: `Save to ${table}${type ? ` as 􀈭 ${type}` : ""}${
        tags
          ? ` with ${tags
              .split(",")
              .map((t) => "􀆃 " + t)
              .join(", ")}`
          : ""
      }${done ? " 􀃲" : ""}${notes ? ` with Notes 􀓕 ` : ""}${list ? ", Add to 􀈕 " + list : ""}`,
      subtitle: title || url,
      arg: " ",
 ...
 * @param {string} Name
 * @param {string} URL
 * @param {object} params - {List, Notes, Done, Tags, Type}
...
const save = async () => {
  let { url, title, table, type, tags, notes, list, done, id } = process.env;
